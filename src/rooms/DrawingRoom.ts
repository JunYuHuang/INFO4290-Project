...
export class DrawingRoom extends Room<DrawingRoomState> {
  ...
  kmpSearch(pattern: string, text: string) {
    if (pattern.length == 0)
  ...
  containsPattern(pattern: string, text: string) {
    let found = false;
    let kmpSearchResult: number = this.kmpSearch(pattern, text);
  ...
  addPointsToDrawer(userID: string) {
  ...
  addPointsToNonDrawer(userID: string) {
  ...
  removeAllWhitespace(text: string) {
  ...
  syncClientsGameStatusState(bool: boolean) {
        ...
        if (currentTurnTime > 0) {
          ...
          if (currentTurnTime === turnStartTime) {
            ...
            if (this.state.game.getTurnInProgress()) {
              let drawerClient: Client = this.getClient(this.state.game.getDrawerID());
